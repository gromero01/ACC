# # Gráfica CMC
  ...
  plotCMC <- function(data, outPathGraph, indexData)
  {
    ...
    CMCtest <- CMCtest + geom_text(aes(label = Removed.Item, angle = 45,
               hjust = -0.2, vjust = -0.2), size = 2.5)
    CMCtest <- CMCtest + geom_linerange(aes(x = MaxX, ymin = MinY, ymax = MaxY))
    ...
    textoAux <- paste0("<b>La máxima confiabilidad es de ", round(max(ConfTot$Alpha.Max), 2),
                       ", esta se obtiene eliminando ",
                       indexItems - ConfTot$MaxX," items.</b>")
    return(textoAux)
  }
