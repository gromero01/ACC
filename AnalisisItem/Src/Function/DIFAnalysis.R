...
DIFAnalysis <- function(confkkBlock, blockStrucUbyIn, 
  ...
  cutDRU <- cortesDiff$DRU

  # # # cut off points for DRN
  cutDRN <- cortesDiff$DRN

  itCompaUs <- paste0("X", itCompa)
  ...
  for(jj in 1:length(listVaInterest)){
    ...
    twoPL <- ifelse(dictVarPruebaIndSub[, "codMod"] %in% c("05", "07"), 1, 0)
    ...
    # # para DRU (DIF UNIFORME)
    u1 <- unique(cutDRU[, "SizeSam"])
    v1 <- abs(nObs - u1)
    w1 <- u1[which.min(v1)]
    # # para DRN (DIF NO UNIFORME)
    ...
    if(length(indicadorDIC) != 0){
      ...
      grouptoDif   <- as.numeric(factor(confkkBlock[, vaInterest]))
      sink(file = "borrar.txt")
      cat("O.o--Voy a iniciar las regresiones logisticas--o.O\n")
      difDic <- func_RL_ambas_fases(datatoDicDif, grouptoDif,
                                    pseudo.R = "McFadden", tipo = "Separado",
                                    use.glm = TRUE, use.nnet = FALSE, itSha = itCompaUs)
     ...
     namesPrueba <- subset(dictVarPruebaIndSub, codigo_prueba == auxiPru, 
     ...
     ################################################################################
     outPathPba <- file.path(outPath, auxiPru)
     if(!dir.exists(outPathPba)) {
      dir.create(outPathPba)
     }
     #dir.create(file.path(outPath, "Invariance07"), showWarnings = FALSE)
     outFile <-  file.path(outPathPba, paste("DIF_", index, ".xlsx", sep = ""))
     ################################################################################
     ...
     infoTable$infoPBA <- rbind(
                           unique(dictVarPruebaIndSub[, "prueba"]),
                           unique(dictVarPruebaIndSub[, "codigo_prueba"]))
    ...
    addDataFrame(namesPrueba, sheet  = get(sheetOut[1]),
     ...
     if(length(tempDIC) == 1) {
       ...
       for(zz in 1:ncol(tempDICtwo)){
           ...
           } else colStyleTwoDIC[[zz]] <- c(NULL) # NULL <- valor anterior
       }
       
       if(length(colStyleTwoDIC) > 0){
         names(colStyleTwoDIC) <- as.character(1:ncol(tempDICtwo))
       }
     ...
     # # Para polit'omicos 
     if(length(tempPOL) == 1) {
       tempPOLone <- "No hay ítems politómicos "
       rownamepol      <- FALSE
       colnamepol      <- FALSE
       colnamStylepol <- NULL
       colStylepol     <- NULL
       ...
       if(length(colStylePOL) > 0){
         names(colStylePOL) <- as.character(1:ncol(tempPOLtwo))
       }
     }
